...
print_large_text("ATTACK ALEX TOOL")
print("")
        ...
        try:
            ...
            async with session.get(url, headers=headers) as response:
                if response.status == 200:
                    print_colored_text('green', f' ATTACK {ip}:{port}')  # طباعة الرسالة عند إرسال طلب بنجاح
                else:
                    print(f"Request to {ip}:{port} failed with status code {response.status}")
